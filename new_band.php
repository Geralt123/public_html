...
<div class="box">
	<div class="box_left">
    	<a href="<?php echo $url_home; ?>">Forum Index</a> &gt; New Band
    </div>
	<div class="box_right">
...
if(isset($_POST['name'], $_POST['description']) and $_POST['name']!='')
{
	...
	if(mysql_query('insert into bands (id, name, description, position) select ifnull(max(id), 0)+1, "'.$name.'", "'.$description.'", count(id)+1 from bands'))
	{
	?>
	<div class="message">The band have successfully been created.<br />
	<a href="<?php echo $url_home; ?>">Go to the forum index</a></div>
	<?php
	}
	else
	{
		echo 'An error occured while creating the band.';
	}
}
else
{
